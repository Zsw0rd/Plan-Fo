...
const BOOKING_RESULTS_CONCURRENCY = 4;
...
async function fetchBookingOptions(offer, context) {
    if (!offer.bookingToken || !offer.segments?.length) {
        return [];
    }

    const selectedLegs = buildSelectedLegs(offer);
...
async function addBookingOptions(offers, context) {
    return mapWithConcurrency(offers, BOOKING_RESULTS_CONCURRENCY, async (offer) => {
        try {
            const bookingOptions = await fetchBookingOptions(offer, context);
            const bestBookingOption = bookingOptions[0];

            return {
                ...offer,
                bookingOptions,
                bookingUrl: bestBookingOption?.bookingUrl || "",
                bookingSellerName: bestBookingOption?.sellerName || "",
                selectedGoogleFlightsUrl: bestBookingOption?.bookingUrl || offer.selectedGoogleFlightsUrl
            };
        } catch (_error) {
            return {
                ...offer,
...
async function searchFlightOffers({ origin, destination, departureDate }) {
    ...
    const bookingContext = { origin, destination, departureDate };
    const direct = await addBookingOptions(
        offers.filter((offer) => offer.type === "direct").slice(0, MAX_RESULTS_PER_CATEGORY),
        bookingContext
    );
    const connecting = await addBookingOptions(
        offers.filter((offer) => offer.type === "connecting").slice(0, MAX_RESULTS_PER_CATEGORY),
        bookingContext
    );
